...
int main(void)
{
	...
	int size = sizeof(ary) / sizeof(ary[0]); // 배열 요소의 갯수 계산
...
double find_max(double* pa, int size)
{
	double max;
	int i;
	max = pa[0];		// 첫번째 배열 요소의 값을 최댓값으로
	for (i = 1; i < size; i++) // 두번째 배열 요소부터 max와 비교
	{
		if (pa[i] > max) max = pa[i];	// 새로운 배열 요소의 값이 max보다 크면 대입함
	}
	return max;
}
...
void input_ary(double* pa, int size) // double 포인터를 매개변수로 선언
{
	...
	{
		scanf("%lf", pa + i);	// &pa[i]도 가능. 입력할 배열요소 주소를 전달
	}
}